...
When the user selected some text and types a formatting character like `_` or `*`, the selected text is formatted by adding the formatting at the beginning and end of the selected text.
The plugin uses constrained/unconstrained formatting as necessary.
If the selected text is already formatting with the chosen formatting, the plugin removes the formatting from the selected text.
