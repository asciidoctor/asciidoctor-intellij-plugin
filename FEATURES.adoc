...
EXPERIMENTAL: When using mathematical formulas in the preview, the preview can flicker.
There is an experimental feature that you can enable in the settings of the plugin to activate a new update mechanism that avoids the flickering. Activate the option "`Replace preview contents without flicker (JavaFX preview only)`" for this.
