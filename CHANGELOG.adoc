...
- Resolve Antora `example$` resource ids as Kroki diagram block macro targets inside an Antora module, and show a placeholder with an open-in-editor link for Kroki diagrams the preview cannot render (unresolvable target, missing include file) instead of an error paragraph or a silently degraded diagram (#516)
- Update the embedded Kroki extension to asciidoctor-kroki 2.0.0.rc.3: PlantUML `!include`, `!includesub`, `!include_once`, `!include_many` and `!includeurl` directives (relative to the diagram file, plus `kroki-plantuml-include-paths`) are now resolved before the diagram is sent to Kroki, a failed diagram no longer aborts the page, and block macro targets resolve relative to the document (#516, thanks to @cachescrubber)
- Language running improved (#2101, thanks to @JanDiederich):
* Better interpreter finding.
* Interpreters configurable.
* Go, TypeScript execution added.
* Rare race-condition fixed when running a script.
...
- Added code execution and code copying to the JCEF Preview, code execution support for JavaScript, PowerShell, Python and Ruby,  (#2038, thanks to @JanDiederich)

=== 0.45.4

- Avoid iterating to parents outside of current document (#1975)
- Bump org.asciidoctor:asciidoctorj-diagram from 3.1.0 to 3.2.1 (#2008)
- Bump org.asciidoctor:asciidoctorj-diagram-batik from 1.17 to 1.19 (#2003)
- Bump org.asciidoctor:asciidoctorj-diagram-plantuml from 1.2025.3 to 1.2026.2 (#1989)
- Fix injection of PHP language in source blocks (#1990)
