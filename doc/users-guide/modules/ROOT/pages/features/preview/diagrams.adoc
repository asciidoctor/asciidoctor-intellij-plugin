...
[NOTE]
====
Only when a SVG or diagram is inlined, the plugin will redirect clicks on links in the preview to local AsciiDoc files.
====
...
NOTE: The preview of the diagram in this documentation is non-interactive, as the target link of the note would not resolve.
