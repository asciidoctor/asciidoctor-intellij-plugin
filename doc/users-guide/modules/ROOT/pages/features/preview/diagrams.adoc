...
:description: The preview of the AsciiDoc plugin supports rendering the diagrams in the preview. It supports Asciidoctor Diagram and Kroki.
...
When running in Asciidoctor UNSAFE mode, the plugin creates a temporary folder outside the project.
...
See below for example _.gitignore_ file placed in the root of the project if the project uses Git as version control system.
