...
When using listings with `[source,lang]` and the user moves the mouse over them, 2 arrows appear in the top left corner, so the user can execute that script.
...
Unsupported languages will just be ignored, even if the arrows appear.
