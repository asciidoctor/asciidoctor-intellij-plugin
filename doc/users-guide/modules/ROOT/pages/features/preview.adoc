...
When using listings with `[source,lang]` and the user moves the mouse over them, 2 arrows appear in the top left corner, so the user can execute that script, if the language is supported.
Currently supported languages are Bash, Go, Java, JavaScript, PowerShell, Python, Ruby and TypeScript.
Java _and_ JShell support need the JShell plugin to be installed and enabled.

To configure some of the supported languages, go to the settings for this plugin,
and click the "Configure script language execution..." button at the bottom.

Here you can configure the path to the interpreter, additional parameters for the interpreter, and if a language should be executed via temp-file, and not via inline evaluation (great for script debugging).

Over every source code block, on mouse hovering over it,
a copy button appears in the top left corner,
that can be clicked to copy the source code to the clipboard.
