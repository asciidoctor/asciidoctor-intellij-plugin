...
The user can choose _Create PDF from current file_ from the editor's action toolbar to convert the file in the current AsciiDoc editor to a PDF using Asciidoctor PDF version 1.6.x.
If creating the PDF succeeds, the plugin opens the PDF in the system's PDF viewer.
