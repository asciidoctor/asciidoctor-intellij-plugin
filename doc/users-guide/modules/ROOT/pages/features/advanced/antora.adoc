...
When Kroki is enabled, the plugin also resolves an `example$` resource id used as the target of a diagram
block macro, `+plantuml::example$dir/file.puml[]+`; includes inside the diagram file are plain relative
PlantUML includes, resolved by the Kroki extension.
