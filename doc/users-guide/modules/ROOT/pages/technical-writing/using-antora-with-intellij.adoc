...
+
When only a single content repository is checked out, links to other components will not show the page title, can't be validated in the IDE and can't be navigated to.
...
Continue to xref:./long-lasting-decisions-antora.adoc[] for the next step of this series.
