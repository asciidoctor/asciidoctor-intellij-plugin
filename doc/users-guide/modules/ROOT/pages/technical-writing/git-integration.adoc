...
When a merge conflict occurs, it is usually good to start with menu:Apply non-conclicting changes[] first, either with the menu:All[] option or the magic wand icon image:technical-writing/magicResolveToolbar.svg[].
