...
PlantUML primarily uses the Graphviz dot program to automatically determine the position and structure of elements in most diagrams. While PlantUML offers alternative layout engines, users sometimes need to install Graphviz separately depending on their operating system.

If any PlantUML is rendered incorrectly in the IntelliJ Asciidoc Preview window when using AsciidocJ Diagram then consider installing the graphviz `dot` program using one of the following commands:
