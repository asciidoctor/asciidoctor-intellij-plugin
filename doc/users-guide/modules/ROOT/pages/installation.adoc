...
OpenJDK included with for example a Linux distribution usually don't include JavaFX.
...
If the machine where the plugin should be installed doesn't have a direct internet access, download it on a different machine with Internet access and transfer the file internally via file transfer.
